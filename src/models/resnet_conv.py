...
class ResNet(nn.Module):
    def __init__(self, block, layers, num_classes=16, num_channels = 3, input_width=256, nois_stddev=0, output_key='3D'):
        ...
        self.conv_end = nn.Conv2d(256, num_classes, kernel_size=1, stride=1, padding=0, bias=True)
        ...
        self.conv_end.bias.data *= 0
        self.conv_end.weight.data *= 0.01

    def _make_layer(self, block, planes, blocks, stride=1):
        downsample = None
        #print('make layer icinde stride', stride)
    ...
    def forward(self, x):
        ...
        #1x1 conv after the final relu in layer4
        x = self.conv_end(x)
...
def resnet18(pretrained=False, **kwargs):
    ...
    if pretrained:
        training.utils.transfer_partial_weights(model_zoo.load_url(model_urls['resnet18']), model)
