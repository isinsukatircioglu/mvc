...
class UpBlockWithSkip(nn.Module):
    ...
    def forward(self, x, bridge):
        up = self.up(x)
        if (up.shape[2] != bridge.shape[2]) or up.shape[3] != bridge.shape[3]:
            offset1 = up.size()[2] - bridge.size()[2]
            offset2 = up.size()[3] - bridge.size()[3]

            if offset1 >= 0:
                if offset1 % 2 == 0:
                    padding11 = [offset1 // 2, offset1 // 2]
                else:
                    padding11 = [offset1 // 2, offset1 // 2 + 1]
                padding12 = [0, 0]
            else:
                if offset1 % 2 == 0:
                    padding12 = [-offset1 // 2, -offset1 // 2]
                else:
                    padding12 = [(-offset1) // 2, (-offset1) // 2 + 1]
                padding11 = [0, 0]
            if offset2 >= 0:
                if offset2 % 2 == 0:
                    padding21 = [offset2 // 2, offset2 // 2]
                else:
                    padding21 = [offset2 // 2, offset2 // 2 + 1]
                padding22 = [0, 0]
            else:
                if offset2 % 2 == 0:
                    padding22 = [-offset2 // 2, -offset2 // 2]
                else:
                    padding22 = [(-offset2) // 2, (-offset2) // 2 + 1]
                padding21 = [0, 0]

            bridge = F.pad(bridge, padding21 + padding11)
            up = F.pad(up, padding22 + padding12)
